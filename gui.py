#!/usr/bin/env python3
"""
gui.py — Standalone TOML editor and simulation launcher.
...
# ── TOML serializer (no external deps) ───────────────────────────────────────
...
def build_toml(cfg: dict) -> str:
    ...
    for k in ("wideband", "nl_tables", "sweep", "sweep_table"):
        if o.get(k): kv(k, o[k], 12)
    ln()
    ...
    for carr in cfg.get("carrier", []):
        ln("[[carrier]]")
        for k in ("name","symbol_rate","sps","rolloff","filter_span",
                  "num_symbols","power_db","freq","sweep_demod"):
            if k in carr: kv(f"{k:12}", carr[k])
        ch = carr.get("channel")
        if ch:
...
class CarrierFrame(ttk.LabelFrame):
    ...
    def __init__(self, parent, on_remove, data: dict, **kw):
        ...
        self._sweep_demod = tk.BooleanVar(value=data.get("sweep_demod", True))
        self._build(data)
    ...
    def _build(self, d: dict):
        ...
        for i, (key, label, typ, dflt) in enumerate(self._MAIN):
            ...
            if key == "name":
                var.trace_add("write", lambda *_, v=var: self.configure(text=v.get() or "carrier"))

        ch_row = (len(self._MAIN) // 2) + 2
        ttk.Checkbutton(self, text="Include in sweep demod",
                        variable=self._sweep_demod).grid(
            row=ch_row, column=0, columnspan=2, sticky="w", pady=(6, 0))
        ttk.Checkbutton(self, text="Channel impairments", variable=self._has_ch,
                        command=self._toggle_ch).grid(
            row=ch_row, column=2, columnspan=2, sticky="w", pady=(6, 0))
        self._ch_frame = ttk.Frame(self, padding=(14, 0, 0, 0))
        self._ch_frame.grid(row=ch_row + 2, column=0, columnspan=4, sticky="ew")
        if "channel" in d:
    ...
    def _populate_ch(self, ch: dict):
        ...
        ttk.Checkbutton(self._ch_frame, text="Enabled",
                        variable=self._ch_enabled).grid(row=0, column=0, columnspan=4, sticky="w")
    ...
    def to_dict(self) -> dict:
        ...
        if self._has_ch.get() and self._ch_vars:
            ch = {"enabled": bool(self._ch_enabled.get())}
...
class App:
    ...
    def _build_ui(self):
        ...
        ttk.Entry(tb, textvariable=self._path_var, width=38,
        ...
        ttk.Button(tb, text="▶  Launch Simulation",
                   command=self._launch).pack(side="left", padx=2)
        ...
        ttk.Label(self.root, textvariable=self._status, anchor="w",
                  foreground="gray").pack(fill="x", side="bottom", padx=8, pady=2)
    ...
    def _build_sweep_output_tab(self, nb):
        ...
        _lf(f, "Wideband plot:", r, 0)
        _ent(f, self._sv("out.wideband"), r, 1);  r += 1
        _lf(f, "NL tables plot:", r, 0)
        _ent(f, self._sv("out.nl_tables"), r, 1);  r += 1
        _lf(f, "Sweep plot:", r, 0)
        _ent(f, self._sv("out.sweep"), r, 1);  r += 1
        _lf(f, "Sweep table:", r, 0)
        _ent(f, self._sv("out.sweep_table"), r, 1);  r += 1
        f.columnconfigure(1, weight=1)
    ...
    def _build_carriers_tab(self, nb):
        ...
        ttk.Label(tb, text="Carriers are processed in listed order.",
    ...
    def _add_carrier(self, data: dict | None = None):
        ref: list[CarrierFrame] = [None]  # filled below to close over cf
    ...
    def _collect(self) -> dict:
        ...
        for k, vk in (("wideband","out.wideband"),("nl_tables","out.nl_tables"),
                      ("sweep","out.sweep"),("sweep_table","out.sweep_table")):
            val = sv(vk)
    ...
    def _launch(self):
        try:
            ...
            proc = subprocess.Popen(
                [sys.executable, str(main_py), str(self.path)],
                cwd=str(Path(__file__).parent))
            self._status.set(f"Launched main.py (PID {proc.pid}) — check terminal for output.")
        except Exception as e:
...
if __name__ == "__main__":
    ...
    root = tk.Tk()
    try:
        root.tk.call("tk", "scaling", 1.25)  # DPI boost on Windows HiDPI
    except Exception:
        pass
    App(root, toml_path)
    root.mainloop()
