...
def emit_table(L: list[str], header: str, d: dict,
    ...
    """Append a `header` line followed by ``key = value`` lines.

    Keys are padded so the ``=`` signs align within the table.  None-valued
    keys are skipped; `keys` (when given) restricts and orders the emitted
    keys, otherwise the dict's own key order is used.  List values render
    as inline arrays.
    """
